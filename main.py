...
class Deck:
    def __init__(self):
        self.cards = []
        suits = ["hearts", "spade", "clove", "diamond"]
        ranks = [
            {"rank": "A", "value": "11"},
            {"rank": "2", "value": "2"},
            {"rank": "3", "value": "3"},
...
class Hand:
    ...
    def display(self,show_all_dealer_cards=False):
        ...
        message = "Dealer's" if self.dealer else "Your"
        print(f"{message} hand:")

        for index,card in enumerate(self.cards):
            if index == 0 and self.dealer == True and not show_all_dealer_cards and not self.is_blackjack() :
                print("hidden")
            else:
                print(card) 
    ...
    def check_winner(self,player_hand : Hand,dealer_hand : Hand,game_over=False):
        if not game_over:
            if player_hand.get_value() > 21:
                ...
                print("Both players have blackjack ! Game Drew :|")
                return True
